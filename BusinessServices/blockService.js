    ...
    blockService.getBlock = function(blockList, role, blockNumber){
        ...
        .then((query_responses) => {
            ...
            if(blockNumber > 0){
                ...
                if (query_responses.data.data[0].payload.data.actions[0].payload.action.proposal_response_payload.extension.results.ns_rwset && query_responses.data.data[0].payload.data.actions[0].payload.action.proposal_response_payload.extension.results.ns_rwset.length > 1){
                    var reads = new Array();
                    var writes = new Array();
                    query_responses.data.data[0].payload.data.actions[0].payload.action.proposal_response_payload.extension.results.ns_rwset.forEach(element => {
                        element.rwset.reads.forEach(rw => {
                            reads.push(rw);
                        });
                        element.rwset.writes.forEach(rw => {
                            writes.push(rw);
                        });
                    });
                    response.readWrites = {reads: reads, writes: writes};
                }
                else{
