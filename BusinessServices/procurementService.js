...
module.exports = function (fabric_client, channels, peers, eventHubPeers, orderer, users) {
        ...
        .then((user_from_store) => {
            ...
            return invokeChainCode.invokeChainCode(fabric_client, 
                ...
                [
                    ...
                    purchaseOrder.supplierContactAdd,
                    ...
                    orderedMaterial,
                    "Approved",
                    purchaseOrder.updatedBy.toString(),
                    new Date().toString()              
                ]                
            );                
        }).then((results) => {
        ...
        .then((user_from_store) => {
            ...
            return invokeChainCode.invokeChainCode(fabric_client, 
                ...
                [
                    ...
                    goodsReceipt.purchaserContactPerson,
                    goodsReceipt.purchaserContactAdd,
                    goodsReceipt.purchaserContactPhone,
                    goodsReceipt.purchaserContactEmail,
                    goodsReceipt.deliverToPersonName,
                    goodsReceipt.deliveryAddress,
                    totalOrderAmount.toString(),
                    orderedMaterial,
                    goodsReceipt.stockType,
                    goodsReceipt.plant,
                    goodsReceipt.storage
                ]                
            );                
        }).then((results) => {
            return results;
    ...
    procurementService.saveInspectionLot = function(inspectionLot){
        console.log("saveInspectionLot");
        inspectionLot.status = "completed";
        return fabric_client.getUserContext(users.procurementUser.enrollmentID, true)
        .then((user_from_store) => {
            helper.checkUserEnrolled(user_from_store);            
            return invokeChainCode.invokeChainCode(fabric_client, 
    ...
    procurementService.getAllInspectionLotDetails = function(option, value){
        console.log("getAllInspectionLotDetails");
        return fabric_client.getUserContext(users.procurementUser.enrollmentID, true)
        .then((user_from_store) => {
            helper.checkUserEnrolled(user_from_store);
            return queryChainCode.queryChainCode(channels.procurementChannelPC, 
                procurementConfig.channels.procurementchannel.chaincodeId, 
                "getAllInspectionLotDetails", 
                [option, value]);
        }).then((results) => {
            return results;
        }).catch((err) => {
